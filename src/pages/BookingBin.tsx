    ...
    <div className="space-y-6 pb-8">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-2 self-start">
          {isSuperAdmin && selectedBookings.length > 0 ? (
            <button
              type="button"
              disabled={bulkPurging || Boolean(busyRef)}
              onClick={() => setPurgeTargets(selectedBookings)}
              className="inline-flex items-center gap-1.5 rounded-lg border border-rose-200 bg-rose-50 px-3 py-1.5 text-xs font-semibold text-rose-700 hover:bg-rose-100 disabled:opacity-50 transition-all"
            >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 shadow-xs hover:bg-slate-50 hover:text-slate-900 transition-all"
          >
          ...
          <table className="w-full min-w-[860px] text-left text-xs">
                ...
                {isSuperAdmin ? (
                  <th className="px-3 py-2.5 w-10">
                    <input
                      ...
                      disabled={items.length === 0 || bulkPurging}
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading && items.length === 0 ? (
                ...
                <tr>
                  <td colSpan={colSpan} className="px-4 py-12 text-center text-xs text-slate-500 font-medium">
                  ...
                  <tr key={booking.id} className="hover:bg-slate-50/60 transition-colors">
                    {isSuperAdmin ? (
                      <td className="px-3 py-2.5">
                        <input
                          ...
                          disabled={bulkPurging || busyRef === booking.bookingRef}
                          className="h-3.5 w-3.5 rounded border-slate-300 text-lime-600 focus:ring-lime-500/30"
                        />
                      </td>
                    ) : null}
                    ...
                    <td className="px-4 py-2.5">
                      <div className="flex justify-end gap-1.5">
                        <button
                          type="button"
                          disabled={busyRef === booking.bookingRef || bulkPurging}
                        ...
                        {isSuperAdmin ? (
                          <button
                            type="button"
                            disabled={busyRef === booking.bookingRef || bulkPurging}
                            onClick={() => setPurgeTargets([booking])}
